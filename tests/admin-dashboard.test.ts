...
describe("administrator dashboard contracts", () => {
  beforeEach(() => { vi.clearAllMocks(); mocks.getCurrentProfile.mockResolvedValue(admin); mocks.hasRole.mockReturnValue(true); for (const read of [mocks.listUsers, mocks.listProperties, mocks.listPayments, mocks.listAuditEvents]) read.mockResolvedValue(emptyPage); });
  ...
  it.each([getUsers, getProperties, getPayments, getAudit])("rejects guests", async (handler) => { mocks.getCurrentProfile.mockResolvedValue(null); expect((await handler(new Request("http://localhost/api/admin/resource"))).status).toBe(401); });
  it.each([getUsers, getProperties, getPayments, getAudit])("rejects suspended administrators", async (handler) => { mocks.getCurrentProfile.mockResolvedValue({ ...admin, accountStatus: "Suspended" }); expect((await handler(new Request("http://localhost/api/admin/resource"))).status).toBe(403); });
});
