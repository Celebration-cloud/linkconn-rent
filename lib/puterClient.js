...
export async function uploadFile({ path = "/uploads", file }) {
  ...
  try {
    ...
    const share = await puter.fs.share(dest); // make it public
...
export async function runWorker(scriptOrName, payload = {}) {
  ...
  try {
    const module = await import(`@/puter/${scriptOrName}.js`);
    const fn =
      module.default || module[scriptOrName] || Object.values(module)[0];
