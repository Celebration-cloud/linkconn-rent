...
export async function aiChat({ messages, systemPrompt } = {}) {
  ...
  const text = typeof res === "string" ? res : (res?.output ?? res?.text ?? null);
...
export async function generateImage({ prompt, size = "1024x1024" } = {}) {
  ...
  const url = res?.imageUrl || res?.url || (typeof res === "string" ? res : null);
  return { url, raw: res };
}
...
export async function runWorker(scriptOrName, payload = {}) {
  ...
  try {
    ...
    const fn =
      importedModule.default || importedModule[scriptOrName] || Object.values(importedModule)[0];
