...
// These schemas handle validation for Landlord, Agent, and Tenant onboarding flows.
...
// 1. Agent Identity
// Used to validate agent’s personal details and uploaded identification.
export const agentIdentitySchema = z.object({
  fullName: z.string().min(3, "Full name is required"),
  phone: z.string().min(10, "Enter a valid phone number"),
  email: z.email("Enter a valid email"),
  idType: z.string().min(1, "Select your ID type"),
  idImage: z
    .any()
    .refine((file) => file?.length === 1, "Upload a valid ID image"),
  passport: z
    .any()
    .refine((file) => file?.length === 1, "Upload a passport photograph"),
});

// 2. License Verification
// Used for agency registration and CAC or license verification.
export const agentLicenseSchema = z.object({
  ...
  agencyEmail: z.email("Enter a valid agency email"),
  documents: z
    .any()
    .refine(
      (file) => file?.length >= 1,
      "Upload at least one business document (CAC/License)"
    ),
});

// 3. Payout Setup (Agency)
// Used to verify agency’s payment details.
export const agentPayoutSchema = z.object({
  ...
  confirmOwnership: z.literal(true, {
    errorMap: () => ({
      message: "Confirm this account belongs to your agency",
    }),
  }),
});
