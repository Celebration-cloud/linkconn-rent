...
export async function GET(request: Request) {
  try {
    ...
    return apiSuccess(await AdministrationRepository.listAuditEvents(filters, profile.role), "Audit events loaded");
  } catch (error) { unstable_rethrow(error); if (error instanceof ZodError) return apiError(error.issues[0]?.message ?? "Invalid query", 400); console.error("[GET /api/admin/audit]", error); return apiError("Unable to load audit log", 500); }
}
