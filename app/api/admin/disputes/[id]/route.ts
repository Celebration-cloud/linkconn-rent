...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const limit = await checkAdminRateLimit(request, "admin-dispute-mutation", profile.id, 60, 60 * 60 * 1000);
    if (!limit.allowed) return adminRateLimitResponse("Too many dispute actions. Try again later.", limit.resetTime);
