...
export async function GET(request: Request) {
  try {
    ...
    return apiSuccess(await AdministrationRepository.listUsers(filters, profile.role), "Users loaded");
  } catch (error) { unstable_rethrow(error); if (error instanceof ZodError) return apiError(error.issues[0]?.message ?? "Invalid query", 400); console.error("[GET /api/admin/users]", error); return apiError("Unable to load users", 500); }
}
