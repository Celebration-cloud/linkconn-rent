...
export async function GET(request: Request) {
  try {
    ...
    return apiSuccess(await AdministrationRepository.listPayments(filters, profile.role), "Payments loaded");
  } catch (error) { unstable_rethrow(error); if (error instanceof ZodError) return apiError(error.issues[0]?.message ?? "Invalid query", 400); console.error("[GET /api/admin/payments]", error); return apiError("Unable to load payments", 500); }
}
