...
export default async function ViewingDetailPage({ params }: { params: Promise<{ id: string }> }) { const profile = await getCurrentProfile(); if (!profile) redirect("/login?next=/dashboard/viewings"); const { id } = await params; const [items, detail] = await Promise.all([ViewingRepository.list(profile), ViewingRepository.detail(profile, id)]); if (!detail) notFound(); return <ViewingWorkspace role={profile.role} items={items} detail={detail} />; }
