    ...
    <div className="admin-canvas">
      ...
      <div className="mt-4 grid min-h-[34rem] min-w-0 border border-[#d6ddd5] bg-white lg:grid-cols-[23rem_minmax(0,1fr)]">
        ...
        <section className="min-w-0 p-4 sm:p-6">
          {selected ? <>
            <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[#d6ddd5] pb-5"><div className="min-w-0"><div className="flex items-center gap-2"><StatusBadge value={selected.status} />{"priority" in selected && <StatusBadge value={selected.priority} />}</div><h2 className="mt-3 break-words text-2xl font-extrabold tracking-[-0.025em]">{"subject" in selected ? selected.subject : selected.title}</h2><p className="mt-1 break-all text-sm text-muted">{"reference" in selected ? `${selected.reference} · ${selected.email}` : `${selected.requester.email} · ${selected.property.location}`}</p></div></div>
...
function Empty({ kind, detail = false }: { kind: "support" | "maintenance"; detail?: boolean }) { const Icon = kind === "support" ? Headphones : Wrench; return <EmptyState compact icon={Icon} title={detail ? "Select a record to inspect" : `No ${kind} records found`} description={detail ? "The full operational history, evidence, and next actions will open here." : "Adjust the filters or return when new work enters this queue."} />; }
